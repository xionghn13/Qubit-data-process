...
def compute_colors_2QB(weights):
    clrs = []
    for k in range(weights.shape[0]):
        clr = np.zeros(4)
        for idx in range(weights.shape[1]):
            for idy in range(weights.shape[2]):
                clr += weights[k, idx, idy] * np.array(colors[idx % len(colors)]) * np.array(colors[idy % len(colors)])
            # clr += weights[k,idx] * np.array(colors[idx % len(colors)])
        clrs.append(tuple(np.clip(clr, 0., 1.)))
    return clrs


def colored_lines(x, levels, weights, initial, final, num_photons=1, color2QB=False):
    lines = []
    if initial == 0 and num_photons == 1:
        ...
        width = 1.5
        style = '--'
    elif initial == 2 and num_photons == 1:
        width = 1.5
        style = '-.'
    else:
        width = 1.5
    ...
    for idx in range(initial + 1, final):
        ...
        if color2QB:
            lines.append(LineCollection(segments,
                                        colors=compute_colors_2QB(weights[:, idx]),
