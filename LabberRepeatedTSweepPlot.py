...
for i, RabiFile in enumerate(RabiFileList):
    ...
    if MeasurementType == 't1t2interleaved':
        ...
        if Calibration:
            RComplex = sbf.FPSweepBackgroundCalibrate(ReadoutFreq, ReadoutPower, ComplexVoltage, BackFreq, BackComplex,
                                                      BackPower)
        else:
    ...
    for j, trial in enumerate(Counter):
        ...
        if i == 0 and j == 0:
            ...
            if MeasurementType == 't1t2interleaved':
                for ind in range(2):
                    OptMatrixList[ind] = np.concatenate((OptMatrixList[ind], np.reshape(optList[ind], (len(opt), 1))),
                                                        axis=1)
